    ...
    public partial class Order : Page
    {
        ...
        private void OrderDetailDBUpdate(int index)
        {
            using (EATSEntities db = new EATSEntities())
            {
                ...
                {
                    var menuCode = db.Menutbl.FirstOrDefault(m => m.MenuName.Equals(item.MenuName)).MenuCode;
                    db.OrderDetailtbl.FirstOrDefault(o => o.OrderCode.Equals(item.OrderCode) && o.MenuCode.Equals(menuCode)).OrderComplete = true;
                }
                db.SaveChanges();
